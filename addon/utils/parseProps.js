...
async function parseMediaImage(type, id, imagePath, language, rpdbkey, mediaType = "poster", rpdbMediaTypes = null, topposterskey = null, toppostersConfig = null, tmdbPosterSize = "w500") {
  ...
  if (topposterskey && mediaType === "poster") {
    return getTopPostersUrl(type, id, topposterskey, toppostersConfig || {}, language);
  }
...
function getTopPostersUrl(type, id, topposterskey, toppostersConfig = {}, language = 'en-US') {
  // TopPoster format: https://api.top-streaming.stream/{api_key}/{source}/{poster_type}/{id}.jpg
  // TMDB requires prefix: movie-155 or series-1399

  const prefix = type === 'movie' ? 'movie-' : 'series-';
  const posterType = toppostersConfig.posterType || 'poster-default';
  const style = toppostersConfig.style || 'modern';
  ...
  // Add style if not modern (default)
  if (style && style !== 'modern') {
    params.append('style', style);
  }

  const queryString = params.toString();
  const baseUrl = `https://api.top-streaming.stream/${topposterskey}/tmdb/${posterType}/${prefix}${id}.jpg`;
